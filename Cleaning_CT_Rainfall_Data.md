...
Before beginning, it is helpful to start with the uncompressed data
files in a single file location. In my case I have placed them in a
folder called 'Data' which is located in the working directory.
...
From there, the precipitation needs to be parsed from the data storage
...
using ungroup(), mutate() and mean(), and then calculating each year's
